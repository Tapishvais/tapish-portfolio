  ...
  return (
    <section id="experience" className="container relative mx-auto max-w-6xl px-4 py-24 sm:py-32">
      ...
      <div className="relative">
        <div className="absolute left-4 top-2 h-full w-[1.5px] bg-gradient-to-b from-indigo-500/60 via-blue-500/40 to-sky-400/40 md:left-1/2" />
          ...
          {EXPERIENCE.map((item, i) => (
            ...
            >
              <div className={`relative pl-12 md:pl-0 ${i % 2 === 0 ? 'md:pr-10 md:text-right' : 'md:pl-10'}`}>
                <div className="absolute left-[7px] top-1.5 grid h-5 w-5 place-items-center rounded-full bg-gradient-to-br from-indigo-500 to-sky-500 shadow-[0_0_0_4px_hsl(240_10%_5%)] md:left-1/2 md:-translate-x-1/2">
                  ...
                  <ul className="mt-4 space-y-2 text-sm text-muted-foreground">
                    {item.highlights.map((h) => (
                      <li key={h} className="flex gap-2">
                        <span className="mt-2 h-1 w-1 shrink-0 rounded-full bg-indigo-400" /> <span>{h}</span>
                      </li>
                    ))}
                  </ul>
